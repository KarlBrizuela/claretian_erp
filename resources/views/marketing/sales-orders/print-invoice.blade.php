            ...
            <table class="items-table">
                ...
                <tbody>
                    @forelse($order->items as $item)
                        @php
                            $desc = $item->book?->name ?? ($item->bundle?->name ?? ($item->product?->name ?? 'Product Item'));
                            $qty = (float) $item->quantity;
                            $price = (float) $item->price;
                            $subtotal = $item->subtotal > 0 ? (float)$item->subtotal : ($qty * $price);
                        @endphp
                        <tr>
        ...
        <div>
            ...
            <div class="payment-sales-row">
                ...
                <div class="total-sales-box">
                    TOTAL SALES: <span>₱{{ number_format($order->total_amount, 2) }}</span>
                </div>
            </div>
