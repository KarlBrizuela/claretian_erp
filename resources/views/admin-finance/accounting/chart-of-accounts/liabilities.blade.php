<div class="card shadow-sm border-0 mb-4" style="border-radius: 8px; border: 1px solid #e2e8f0; background: #ffffff;">
    <div class="card-header bg-white border-0 pt-3 pb-2 px-3 d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h5 class="mb-0 fw-bold fs-16" style="color: #000000;">Liability Accounts</h5>
                <p class="text-muted small mb-0">Financial obligations and debts owed to suppliers or third parties.</p>
            </div>
        </div>
    ...
    <div class="card-body p-3 pt-1">
        <div class="row g-2">
            ...
            <div class="col-xl-3 col-md-4 col-sm-6">
                <div class="card h-100 shadow-sm hover-card liabilities-card" style="background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 10px; transition: all 0.2s ease; cursor: pointer;" onclick="openAccountLedgerModal({{ $acc->id }}, '{{ $acc->code }}', '{{ addslashes($acc->name) }}')">
                    <div class="card-body p-3 d-flex flex-column justify-content-between">
                        <div>
                            ...
                            <p class="text-muted small mb-3" style="font-size: 0.76rem; line-height: 1.4; min-height: 38px;">{{ $acc->category ?: 'Liability Account' }}</p>
                        </div>
            ...
            @empty
            <div class="col-12 py-4 text-center text-muted">No liability accounts registered in the database.</div>
            @endforelse
        </div>
    </div>
</div>
